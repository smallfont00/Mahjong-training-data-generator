...
def generateImage(loadPath, savePath):
    fileName = glob.glob(os.path.join(loadPath, '*.jpg'))
    counts = len(fileName)
    images = []
    ...
    if mode == 0:
        ...
        aug.add(iaa.PiecewiseAffine(scale=(0.02),
                                    nb_cols=(2, col), nb_rows=(2, row)))
    ...
    for name in fileName:
        if name[-3:] == 'jpg':
            cv2.imwrite(os.path.join(savePath, name),
                        batches_aug[0].images_aug[i])
            i = i + 1
